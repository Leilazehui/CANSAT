...
const byte ACCEL_CONFIG = 0x1C; // ACCEL_CONFIG  register address of the MPU_6050
const byte ACCEL_XOUT_H = 0x3B; // accel_xout_h REGISTER ADDRESS OF THE MPU 6050
const byte GYRO_CONFIG = 0x1B; // temp_out_h register address of the mpu-6050
...
void initSensors() {
  ...
  Wire.write(0);    // set to zero to wake up the MPU_6050
  ...
  Wire.write(0x08);     // setting the gyro full scale range to plus minus 250degree/s
  Wire.endTransmission();
  bme.begin(0x76);
...
void printSensors() {
  ...
  Serial.print("MPU-6050 Gyro X-Axis   "); Serial.print(MPU_6050_GyroX, 2); Serial.println(" [g]");
  Serial.print("MPU-6050 Gyro Y-Axis   "); Serial.print(MPU_6050_GyroY, 2); Serial.println(" [g]");
  Serial.print("MPU-6050 Gyro Z-Axis   "); Serial.print(MPU_6050_GyroZ, 2); Serial.println(" [g]");
  Serial.print("MPU-6050 Temperature   "); Serial.print(MPU_6050_Temp, 1); Serial.println(" [degree C]");
  Serial.println(" ---------- ");
  Serial.print("BMP280 Temperature      = "); Serial.print(bme.readTemperature()); Serial.println(" [*C]");
  Serial.print("BMP280 Pressure         = "); Serial.print(bme.readPressure()/100); Serial.println("  [hPa]");
  Serial.print("BMP280 Approx altitude  = "); Serial.print(bme.readAltitude(1013.25)); Serial.println(" [m]");
  Serial.println(" ---------- ");
  delay(1000);
